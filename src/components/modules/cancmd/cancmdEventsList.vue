...
import CancmdEventVariables from "components/modules/cancmd/cancmdEventVariables";
...
const update_rows = () => {
  console.log(`CancmdEventList Mounted ${store.state.selected_node}`)
  rows.value = []
...
const editEvent = (eventIndex) => {
  ...
  store.methods.update_event_component("CancmdEventVariables")
}
